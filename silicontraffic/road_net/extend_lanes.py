...
def build_extended_lanes(road_net: RoadNet, min_length: float = 50) -> RoadNet:
    """
    Build "extended lanes" for every traffic light of the given road net.

    For every signalized junction, each incoming lane whose own length is
    shorter than `min_length` and whose upstream junction is **not**
    signalized is extended upstream (recursively, through every upstream
    lane) until the accumulated approach length reaches `min_length` (or the
    chain is blocked by another signal / the end of the network).
        ...
        for junction in tl.junctions:
            for lane in junction.incoming_lanes:
                if lane.id in extended:
                    continue
                ext = _extend_incoming(lane, tl_by_junction, min_length)
    ...
    # ---- pass 2: outgoing extended lanes ----
    outgoing_lane_map: dict[str, LaneLike] = {}
    for tl in road_net.traffic_lights:
        for junction in tl.junctions:
            for lane in junction.outgoing_lanes:
                outgoing_lane_map[lane.id] = participating.get(lane.id, lane)
...
def _extend_incoming(
    ...
    chain reaches `min_length`. Returns None when no upstream extension is
    needed (i.e. the lane itself is already long enough).
    """
